...
describe("bazaarIdentifiers", () => {
  test("buildBazaarRid does not use sign(null, digest) — EC P-256 PKCS#8 (OpenSSL3-safe)", () => {
    ...
    const prev = process.env.APP_SERVICE_PRIVATE_KEY;
    process.env.APP_SERVICE_PRIVATE_KEY = pem;
    try {
      ...
      if (prev === undefined) delete process.env.APP_SERVICE_PRIVATE_KEY;
      else process.env.APP_SERVICE_PRIVATE_KEY = prev;
    }
  });
});
