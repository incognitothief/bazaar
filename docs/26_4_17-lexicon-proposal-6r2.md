# Bazaar — ATProto Commerce Layer: Lexicon Proposal v6 rev.2
**Date:** 17 April 2026
**Status:** Working proposal, v6 rev.2 — appDid signing clarification, kid field added to bazaarIdentifier, purchase.receipt, and purchase.consent
...
**`defs#bazaarIdentifier` — signing authority corrected.** The type description and `sig` field description previously stated that identifiers are signed by the artist's ATProto keypair. This was incorrect. ATProto OAuth does not expose the artist's private key to the application. All `bazaarIdentifier` signatures are produced by the Bazaar app service keypair (`appDid`). Ownership is proven transitively: the app signed the identifier using its service keypair and wrote the containing record to the artist's PDS via an OAuth session the artist granted. Verifiers resolve `appDid` — not the artist's DID document — to obtain the public verification key. A future upgrade path exists to client-side artist keypair signing if the ATProto SDK exposes the required signing primitive; this is documented in `26_4_3-identifier-signing-strategy.md`.

**`purchase.receipt#appSig` description** — updated to document the full canonical payload with explicit field order. Previously terse.

**`purchase.consent#appSig` description** — updated to document the full canonical payload with explicit field order. Previously terse.

### Additions

**`defs#bazaarIdentifier#kid`** — new optional string field. Key identifier for the app service keypair that produced `sig`. Format: `app-key-{YYYY-MM-DD}` using the keypair's activation date. Allows verifiers to select the correct historical key from the `appDid` DID document `verificationMethod` array without trying all keys exhaustively after a keypair rotation. `kid` is a hint — verifiers still verify cryptographically and fall back to trying all non-revoked keys if the hinted key fails. Immutable once written. Inherited by all five `bazaarIdentifier` placements (`bazaarRid` on `catalog.item.digital` and `catalog.recording`, `bazaarWid` on `catalog.recording` and `catalog.composition`, `bazaarPid` on `catalog.collection`) via the shared `$ref`.

**`purchase.receipt#kid`** — new optional string field. Same semantics as `defs#bazaarIdentifier#kid` but applied to the receipt-level `appSig`. Receipts on buyer PDSs cannot be updated after writing, making `kid` the only mechanism for efficient key selection after a rotation. Immutable.

**`purchase.consent#kid`** — new optional string field. Same semantics. Consent records on buyer PDSs cannot be updated after writing. Immutable.
...
| `events` | Freely mutable — append-only by convention, not enforced |
| `createdAt` | Immutable |
...
**Updated in v6 rev.2.** `bazaarIdentifier` type description and `sig` field description corrected to reflect `appDid` signing. `kid` field added.
  ...
  "defs": {
    ...
    "bazaarIdentifier": {
      ...
      "properties": {
        "id": {
          "type": "string",
          "description": "The self-issued identifier. Prefixed by type: bazaar:rid:* for recordings, bazaar:wid:* for works/compositions, bazaar:pid:* for products/releases. Immutable."
        },
        "sig": {
          "type": "string",
        ...
        "supersededByIsrc": {
          "type": "string",
          "maxLength": 16,
          "description": "Populated when a proper ISRC is obtained. The bazaar:rid remains valid as the historical assertion record. Mutable."
        },
        "supersededByIswc": {
          "type": "string",
          "maxLength": 16,
          "description": "Populated when a proper ISWC is obtained. The bazaar:wid remains valid as the historical assertion record. Mutable."
        },
        "supersededByUpc": {
          "type": "string",
          "maxLength": 20,
          "description": "Populated when a proper UPC is obtained. The bazaar:pid remains valid as the historical assertion record. Mutable."
        }
      }
    }
  }
...
Unchanged from v6 rev.1. `bazaarRid` inherits `kid` via `$ref` to `defs#bazaarIdentifier`.
      ...
      "record": {
        ...
        "properties": {
          ...
          "bazaarRid": {
            ...
            "description": "Self-issued Recording Identifier. Generated as base32(SHA-256(artistDid + fileCid + fileChecksum + createdAt)). Shadows ISRC. id, sig, kid, and generatedAt are immutable. supersededByIsrc is mutable — populated when a proper ISRC is obtained."
          },
          "collectionUri": {
            "type": "string",
            "format": "at-uri",
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "title": { "type": "string", "maxLength": 512, "description": "Freely mutable." },
          "artistDid": { "type": "string", "format": "did", "description": "Immutable." },
          "itemClass": {
            "type": "string",
            "knownValues": ["clothing", "vinyl", "cd", "cassette", "poster", "print", "accessory", "hardGood", "other"],
            "description": "Immutable."
          },
          "description": { "type": "string", "maxLength": 4096, "description": "Freely mutable." },
          "variants": {
            "type": "array",
          ...
          "artworkCid": { "type": "string", "format": "cid", "description": "Freely mutable." },
          "countryOfOrigin": { "type": "string", "maxLength": 2, "description": "Freely mutable." },
          "harmonizedCode": { "type": "string", "maxLength": 16, "description": "HS tariff code. Freely mutable." },
          "requiresShipping": { "type": "boolean", "default": true, "description": "Freely mutable." },
          "createdAt": { "type": "string", "format": "datetime", "description": "Immutable." }
        }
      }
    }
  }
...
Unchanged from v6 rev.1. Commercial convenience grouping only. No product identity, no `bazaarPid`, no `upc`. Not a release.
...
Unchanged from v6 rev.1. `bazaarPid` inherits `kid` via `$ref` to `defs#bazaarIdentifier`.
      ...
      "record": {
        ...
        "properties": {
          ...
          "description": {
            "type": "string",
            "maxLength": 4096,
            "description": "Optional longer text for the release (e.g. storefront, listings, liner notes). Freely mutable."
          },
          "releaseDate": { "type": "string", "format": "datetime", "description": "Freely mutable." },
          "items": {
            "type": "array",
            "minLength": 1,
          ...
          "bazaarPid": {
            "$ref": "diamonds.whereditgo.bazaar.defs#bazaarIdentifier",
            "description": "Self-issued Product Identifier. Generated as base32(SHA-256(artistDid + collectionUri + releaseDate)). Shadows UPC. id, sig, kid, and generatedAt are immutable. supersededByUpc is mutable — populated when a proper UPC is obtained."
          },
          "artworkCid": { "type": "string", "format": "cid", "description": "Freely mutable." },
          "genre": {
            "type": "array",
    ...
    "main": {
      ...
      "record": {
        ...
        "properties": {
          ...
          "availableFrom": { "type": "string", "format": "datetime", "description": "Freely mutable." },
          "availableUntil": { "type": "string", "format": "datetime", "description": "Freely mutable." },
          "maxPurchasesPerBuyer": { "type": "integer", "description": "Freely mutable." },
          "createdAt": { "type": "string", "format": "datetime", "description": "Immutable." }
        }
      }
    }
...
Unchanged from v6 rev.1. `bazaarRid` and `bazaarWid` inherit `kid` via `$ref` to `defs#bazaarIdentifier`.
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "itemUri": { "type": "string", "format": "at-uri", "description": "Immutable. Anchors this record to a specific item." },
          "itemCid": { "type": "string", "format": "cid", "description": "Immutable." },
          "isrc": {
            "type": "string",
            "maxLength": 16,
          ...
          "bazaarWid": {
            "$ref": "diamonds.whereditgo.bazaar.defs#bazaarIdentifier",
            "description": "Self-issued Work Identifier. Generated as base32(SHA-256(artistDid + compositionTitle + writers[] + createdAt)). Shadows ISWC. id, sig, kid, and generatedAt are immutable. supersededByIswc is mutable — populated when a proper ISWC is obtained."
          },
          "recordingMetaUri": {
            "type": "string",
            "format": "at-uri",
          ...
          "masterOwnerDid": { "type": "string", "format": "did", "description": "Freely mutable." },
          "publishingOwnerDid": { "type": "string", "format": "did", "description": "Freely mutable." },
          "publishingOwnerIpi": { "type": "string", "maxLength": 32, "description": "Freely mutable." },
          "masterLicenseTermsUri": { "type": "string", "format": "at-uri", "description": "Freely mutable." },
          "publishingLicenseTermsUri": { "type": "string", "format": "at-uri", "description": "Freely mutable." },
          "createdAt": { "type": "string", "format": "datetime", "description": "Immutable." }
        }
      }
    }
...
Unchanged from v6 rev.1. `bazaarWid` inherits `kid` via `$ref` to `defs#bazaarIdentifier`.
    ...
    "main": {
      ...
      "record": {
        ...
        "properties": {
          ...
          "bazaarWid": {
            "$ref": "diamonds.whereditgo.bazaar.defs#bazaarIdentifier",
            "description": "Self-issued Work Identifier. Generated as base32(SHA-256(artistDid + compositionTitle + writers[] + createdAt)). Writers array canonicalized before hashing. Shadows ISWC. id, sig, kid, and generatedAt are immutable. supersededByIswc is mutable — populated when a proper ISWC is obtained."
          },
          "writers": {
            "type": "array",
          ...
          "copyrightYear": { "type": "integer", "description": "Freely mutable." },
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "title": { "type": "string", "maxLength": 256, "description": "Freely mutable." },
          "tier": {
            "type": "string",
          ...
          "version": { "type": "string", "maxLength": 32, "description": "Freely mutable." },
          "territoryCoverage": { "$ref": "diamonds.whereditgo.bazaar.defs#territoryCoverage", "description": "Freely mutable." },
          "term": {
            "type": "object",
          ...
          "summary": { "type": "string", "maxLength": 1024, "description": "Freely mutable." },
          "editionSize": { "type": "integer", "description": "For limited editions. Absent = open edition. Freely mutable." },
...
**Updated in v6 rev.2.** `appSig` description updated to document full canonical payload and `appDid` signing authority. `kid` field added.
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "item": { "$ref": "diamonds.whereditgo.bazaar.defs#itemRef", "description": "Immutable." },
          "listingUri": { "type": "string", "format": "at-uri", "description": "Immutable." },
          "listingCid": {
            "type": "string",
            "format": "cid",
            "description": "CID of the listing at time of purchase — immutable price anchor. Immutable."
          },
          "pricePaid": { "$ref": "diamonds.whereditgo.bazaar.defs#money", "description": "Immutable." },
          "paymentProcessor": { "type": "string", "maxLength": 128, "description": "Immutable." },
          "paymentRef": {
            "type": "string",
            "maxLength": 256,
            "description": "Stripe PaymentIntent ID or equivalent. Independently verifiable via payment processor. Immutable."
          },
          "licenseGrantUri": { "type": "string", "format": "at-uri", "description": "Immutable." },
          "licenseGrantCid": {
            "type": "string",
            "format": "cid",
          ...
          "fulfillmentUri": { "type": "string", "format": "at-uri", "description": "Immutable." },
          "appDid": {
            "type": "string",
            "format": "did",
          ...
          "purchasedAt": { "type": "string", "format": "datetime", "description": "Immutable." },
          "note": { "type": "string", "maxLength": 512, "description": "Immutable." }
        }
      }
    }
  }
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "itemUri": { "type": "string", "format": "at-uri", "description": "Immutable." },
          "itemCid": { "type": "string", "format": "cid", "description": "Immutable." },
          "variantSku": { "type": "string", "maxLength": 128, "description": "Immutable." },
          "quantityAvailable": { "type": "integer", "minimum": 0, "description": "Freely mutable — updated by the backend as stock changes." },
          "quantityReserved": { "type": "integer", "minimum": 0, "description": "Freely mutable." },
          "quantitySold": { "type": "integer", "minimum": 0, "description": "Freely mutable." },
          "isUnlimited": { "type": "boolean", "description": "True for digital items or print-on-demand goods. Freely mutable." },
          "lowStockThreshold": { "type": "integer", "description": "Freely mutable." },
          "updatedAt": { "type": "string", "format": "datetime", "description": "Freely mutable — updated with every stock change." }
        }
      }
    }
  }
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "receiptUri": { "type": "string", "format": "at-uri", "description": "Immutable." },
          "receiptCid": { "type": "string", "format": "cid", "description": "Immutable." },
          "status": {
            "type": "string",
          ...
          "carrier": { "type": "string", "maxLength": 128, "description": "Freely mutable." },
          "trackingNumber": { "type": "string", "maxLength": 256, "description": "Freely mutable." },
          "trackingUrl": { "type": "string", "format": "uri", "description": "Freely mutable." },
          "estimatedDelivery": { "type": "string", "format": "datetime", "description": "Freely mutable." },
          "shippedAt": { "type": "string", "format": "datetime", "description": "Freely mutable." },
          "deliveredAt": { "type": "string", "format": "datetime", "description": "Freely mutable." },
          "events": {
            "type": "array",
          ...
          "createdAt": { "type": "string", "format": "datetime", "description": "Immutable." },
          "updatedAt": { "type": "string", "format": "datetime", "description": "Freely mutable." }
        }
      }
    }
  }
  ...
  "defs": {
    "main": {
      ...
      "record": {
        ...
        "properties": {
          "displayName": { "type": "string", "maxLength": 256, "description": "Freely mutable." },
          "description": { "type": "string", "maxLength": 2048, "description": "Freely mutable." },
          "storefrontUrl": { "type": "string", "format": "uri", "description": "Freely mutable." },
          "avatarCid": { "type": "string", "format": "cid", "description": "Freely mutable." },
          "bannerCid": { "type": "string", "format": "cid", "description": "Freely mutable." },
          "createdAt": { "type": "string", "format": "datetime", "description": "Immutable." }
        }
      }
    }
...
All five placements use `$ref` to `defs#bazaarIdentifier` and inherit the `kid` field automatically.
...
1. Obtain `appDid` from the receipt or consent record
2. Resolve `appDid` via the AT Protocol DID document (`/.well-known/did.json`) to obtain available public verification keys
3. Use `kid` from the record to select the correct key from `verificationMethod` — fall back to trying all non-revoked keys if `kid` is absent or the hinted key fails verification
4. Reconstruct the canonical payload from the record's plaintext fields:
   - Receipt: `SHA-256(purchasedAt:paymentRef:item.uri:listingCid:buyerDid)`
   - Consent: `SHA-256(buyerDid:licenseGrantCid:receiptCid:consentedAt)`
5. Verify `appSig` against the reconstructed payload using the resolved public key
6. Confirm `authenticatedSessionDid === receipt.buyerDid` — prevents receipt replay across buyer PDSs
7. Optionally cross-reference `paymentRef` against Stripe to confirm the transaction independently
8. Resolve `licenseGrantUri` + `licenseGrantCid` to confirm the exact license terms in force at time of purchase
9. Resolve the item URI to obtain `fileChecksum` — cross-reference against the file served by the backend to verify storage integrity
10. If `item.itemType` is `catalog.collection`, resolve the collection to enumerate all items covered by this receipt
11. Resolve `purchase.consent` on the buyer's PDS — verify `buyerDid` matches the receipt, `licenseGrantCid` matches, and `appSig` is valid. For commercial and sync tiers, absence of a consent record is a gap in the evidentiary chain

---

## File replacement flow

File replacement is a publishing act, not an edit. New item record written with `supersedes` pointer. Old record never deleted.
...
Lexicons are managed as JSON files in `packages/shared/src/lexicons/` and served from the Hono app.

---

## Known gaps (carried forward)

- Clickwrap consent is recorded via `purchase.consent` but enforcement remains a UI requirement — the lexicon cannot enforce that the UI presented the terms before writing the consent record
...
- `parentListing` cascade is a backend convention — no lexicon constraint prevents a client from archiving a parent listing without cascading to children
...
- **DID document route** — `/.well-known/did.json` must be implemented and deployed before any signed records go to production PDSs. See `26_4_17-did-document-serving.md`.
